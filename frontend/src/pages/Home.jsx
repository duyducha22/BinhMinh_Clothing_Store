...
const ProductCard = ({ product, onQuickView, navigate }) => {
  ...
  return (
    ...
    >
      <div style={{ position: 'relative', overflow: 'hidden' }}>
        {/* Đổi img thành image, id thành _id để khớp MongoDB */}
        <img src={product?.image} alt={product?.name} style={{ width: '100%', height: '320px', objectFit: 'cover', display: 'block' }} />
        {showActions && (
          <div style={{ position: 'absolute', bottom: 0, left: 0, width: '100%', display: 'flex', background: '#000', alignItems: 'center', zIndex: 10 }}>
            <button
              onClick={() => onQuickView(product)}
...
const QuickViewModal = ({ product, onClose, navigate, onAddToCart }) => {
  ...
  // Lấy size và màu thực tế từ MongoDB
  ...
  return (
    ...
    >
      ...
      >
        ...
        >&times;</button>
        <div style={{ flex: 1.2 }}>
          <img src={product?.image} alt={product?.name} style={{ width: '100%', height: '450px', objectFit: 'cover' }} />
        </div>
        <div style={{ flex: 1, textAlign: 'left' }}>
          <h2 style={{ fontSize: '20px', fontWeight: '700', marginBottom: '5px' }}>{product?.name}</h2>
          <p style={{ color: '#888', fontSize: '12px', marginBottom: '15px' }}>Tình trạng: <span style={{ color: '#000' }}>Còn hàng</span></p>
          ...
          <div style={{ marginBottom: '20px' }}>
            <p style={{ fontSize: '12px', fontWeight: '700', marginBottom: '10px' }}>MÀU SẮC: {selectedColor}</p>
            <div style={{ display: 'flex', gap: '8px' }}>
              {availableColors.map(c => (
                <button 
                  ...
                  style={{ 
                    padding: '5px 12px', border: selectedColor === c ? '1px solid #000' : '1px solid #eee',
                    fontSize: '11px', cursor: 'pointer', background: '#fff'
                  }}
                >{c}</button>
              ))}
            </div>
...
const Home = ({ navigate, onAddToCart }) => {
  ...
  const renderSection = (title, subCats) => {
    const slug = SECTION_SLUG[title] || '';
    const filteredItems = Array.isArray(products) 
      ? products.filter(p => p.category === slug).slice(0, 4) 
      : [];
      ...
      <div style={{ marginBottom: '60px' }}>
        ...
        <div style={{ display: 'flex', flexWrap: 'wrap', gap: '2%' }}>
          {filteredItems.map(item => (
            <ProductCard key={item._id} product={item} onQuickView={setSelectedProduct} navigate={navigate} />
          ))}
        </div>
      </div>
    );
  ...
  if (loading) return <div style={{ textAlign: 'center', padding: '100px', fontSize: '14px' }}>ĐANG TẢI CỬA HÀNG...</div>;
    ...
    <main style={{ width: '100%' }}>
      <section style={{ width: '100%', lineHeight: 0 }}>
        <img
          ...
          alt="Banner"
          style={{ width: '100%', height: 'auto', display: 'block' }}
          onError={e => e.target.src = 'https://via.placeholder.com/1920x800?text=Binh+Minh+Store'}
        />
      </section>

      <div style={{ padding: '50px 7%' }}>
        {renderSection("ÁO THU ĐÔNG", ["Áo Nỉ / Áo Thun Dài Tay", "Áo Len", "Áo Khoác"])}
        {renderSection("ÁO XUÂN HÈ", ["Áo Phông", "Áo Polo", "Áo Sơ Mi Ngắn Tay"])}
        {renderSection("QUẦN NAM", ["Quần Dài", "Quần Short"])}
        {renderSection("PHỤ KIỆN", ["Túi / Balo", "Giày Dép", "Dây Lưng", "Mũ", "Tất"])}
      </div>
