...
const TITLES_MAP = {
  'ao-thu-dong': { title: 'Áo Thu Đông', subCats: ['Tất cả', 'Áo Nỉ / Áo Thun Dài Tay', 'Áo Len', 'Áo Khoác', 'Cardigan', 'Áo Hoodie'] },
  'ao-xuan-he': { title: 'Áo Xuân Hè', subCats: ['Tất cả', 'Áo Phông', 'Áo Polo', 'Áo Sơ Mi Ngắn Tay', 'Áo Tank Top', 'Áo Sơ Mi Dài Tay'] },
  'quan': { title: 'Quần Nam', subCats: ['Tất cả', 'Quần Dài', 'Quần Short'] },
  'phu-kien': { title: 'Phụ Kiện', subCats: ['Tất cả', 'Túi/Balo', 'Giày Dép', 'Dây Lưng', 'Mũ', 'Tất'] },
};
...
const FilterDropdown = ({ label, children, isOpen, onToggle }) => (
  <div style={{ position: 'relative' }}>
    <button onClick={onToggle} style={{ display: 'flex', alignItems: 'center', gap: 6, background: 'none', border: '1px solid #ddd', padding: '6px 14px', fontSize: 12, fontWeight: 600, cursor: 'pointer', borderRadius: 2, color: '#333', whiteSpace: 'nowrap' }}>
      {label} <span style={{ fontSize: 9, marginTop: 1 }}>{isOpen ? '▲' : '▼'}</span>
    </button>
    {isOpen && <div style={{ position: 'absolute', top: '110%', left: 0, background: '#fff', border: '1px solid #eee', boxShadow: '0 8px 24px rgba(0,0,0,0.1)', zIndex: 100, minWidth: 220, padding: 16, borderRadius: 4 }}>{children}</div>}
  </div>
);
...
const ProductCard = ({ product, navigate }) => {
  ...
  return (
    <div style={{ cursor: 'pointer', marginBottom: 32 }} onMouseEnter={() => setHovered(true)} onMouseLeave={() => setHovered(false)} onClick={() => navigate(`/product/${product._id}`)}>
      <div style={{ position: 'relative', backgroundColor: '#f5f5f5', overflow: 'hidden' }}>
        <img src={product.image} alt={product.name} style={{ width: '100%', height: '380px', objectFit: 'cover', display: 'block', transition: 'transform .35s', transform: hovered ? 'scale(1.04)' : 'scale(1)' }} onError={e => e.target.src = 'https://via.placeholder.com/300x380?text=Binh+Minh'} />
        <div style={{ position: 'absolute', top: '38%', left: 0, right: 0, textAlign: 'center', fontSize: 10, fontWeight: 700, letterSpacing: 2, color: 'rgba(0,0,0,0.08)', textTransform: 'uppercase', pointerEvents: 'none', userSelect: 'none', lineHeight: 1.8 }}>
          BINH MINH STORE<br /><span style={{ fontSize: 9 }}>| {product._id?.slice(-6) || '...'} |</span>
        </div>
      </div>
      <div style={{ display: 'flex', gap: 5, marginTop: 10, justifyContent: 'center' }}>
        {availableColors.map((colorName) => {
          const colorObj = ALL_COLORS.find(c => c.name === colorName) || { hex: '#ccc' };
          return <div key={colorName} style={{ width: 14, height: 14, borderRadius: '50%', background: colorObj.hex, border: colorObj.border ? '1px solid #ccc' : 'none' }} title={colorName} />;
        })}
      </div>
      <div style={{ textAlign: 'center', marginTop: 8 }}>
        <p style={{ fontSize: 12, color: '#333', margin: '0 0 4px', fontWeight: 500, lineHeight: 1.4 }}>{product.name}</p>
        <p style={{ fontSize: 14, fontWeight: 800, color: '#000', margin: 0 }}>{product.price?.toLocaleString()}đ</p>
      </div>
    </div>
  );
};
...
const CategoryPage = ({ slug, initialSubCat = 'Tất cả', navigate }) => {
  ...
  useEffect(() => {
    const fetchCategoryProducts = async () => {
      try {
        setLoading(true);
        const { data } = await api.get(`/api/products?category=${slug}`);
  ...
  const products = useMemo(() => {
    let list = [...dbProducts];
    if (activeSubCat && activeSubCat !== 'Tất cả') list = list.filter(p => p.subCategory === activeSubCat);
    if (selectedColors.length > 0) list = list.filter(p => p.variants?.some(v => selectedColors.includes(v.color)));
    if (selectedSizes.length > 0) list = list.filter(p => p.variants?.some(v => selectedSizes.includes(v.size)));
    if (selectedPrice.max !== Infinity || selectedPrice.min > 0) list = list.filter(p => p.price >= selectedPrice.min && p.price <= selectedPrice.max);
    
    if (sortBy === 'price_asc') list.sort((a, b) => a.price - b.price);
    if (sortBy === 'price_desc') list.sort((a, b) => b.price - a.price);
    if (sortBy === 'name_asc') list.sort((a, b) => a.name.localeCompare(b.name));
    return list;
  }, [dbProducts, activeSubCat, selectedColors, selectedSizes, selectedPrice, sortBy]);

  if (loading) return <div style={{ padding: '100px', textAlign: 'center' }}>Đang tải danh mục...</div>;
  if (!info) return <div style={{ padding: '80px 10%', textAlign: 'center' }}><h2>Danh mục không tồn tại</h2></div>;

  const hasActiveFilter = selectedColors.length > 0 || selectedSizes.length > 0 || selectedPrice.min > 0;

  return (
    <div style={{ maxWidth: 1280, margin: '0 auto', padding: '20px 5% 80px', fontFamily: "'Segoe UI', sans-serif" }}>
      <style>{`.cat-subcat { font-size: 12px; font-weight: 600; cursor: pointer; color: #888; padding: 4px 12px; border-radius: 20px; transition: .2s; border: 1px solid transparent; } .cat-subcat:hover { color: #000; } .cat-subcat.active { color: #000; border-color: #000; } .filter-checkbox { display: flex; align-items: center; gap: 8px; cursor: pointer; padding: 6px 0; font-size: 13px; color: #444; } .filter-checkbox:hover { color: #000; } .filter-checkbox input { width: 15px; height: 15px; cursor: pointer; accent-color: #000; } @keyframes catFadeIn { from { opacity: 0; transform: translateY(10px); } to { opacity: 1; transform: translateY(0); } } .cat-grid { animation: catFadeIn .3s ease; }`}</style>
      <div style={{ display: 'flex', gap: 8, fontSize: 12, color: '#888', marginBottom: 20, alignItems: 'center' }}>
        <span style={{ cursor: 'pointer', color: '#555' }} onClick={() => navigate('/')}>Trang chủ</span><span>/</span><span style={{ color: '#111', fontWeight: 600 }}>{info.title}</span>
      </div>
      <h1 style={{ fontSize: 26, fontWeight: 900, margin: '0 0 20px', color: '#111', letterSpacing: 0.5 }}>{info.title}</h1>
      <div style={{ display: 'flex', gap: 8, flexWrap: 'wrap', marginBottom: 20 }}>
        {info.subCats.map(cat => ( <span key={cat} className={`cat-subcat ${activeSubCat === cat ? 'active' : ''}`} onClick={() => setActiveSubCat(cat)}>{cat}</span> ))}
      </div>
      <hr style={{ border: 'none', borderTop: '1px solid #eee', marginBottom: 20 }} />
      ...
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 28, flexWrap: 'wrap', gap: 12 }}>
        <div style={{ display: 'flex', alignItems: 'center', gap: 10, flexWrap: 'wrap' }}>
          ...
          <FilterDropdown label={`Màu sắc${selectedColors.length ? ` (${selectedColors.length})` : ''}`} isOpen={openFilter === 'color'} onToggle={() => toggleFilter('color')}>
            <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 4 }}>
              {ALL_COLORS.map(c => (
                <label key={c.name} className="filter-checkbox">
                  <input type="checkbox" checked={selectedColors.includes(c.name)} onChange={() => toggleColor(c.name)} />
          ...
          <FilterDropdown label={`Kích cỡ${selectedSizes.length ? ` (${selectedSizes.length})` : ''}`} isOpen={openFilter === 'size'} onToggle={() => toggleFilter('size')}>
            <div style={{ display: 'flex', gap: 8, flexWrap: 'wrap' }}>
              {['M', 'L', 'XL', '2XL', '3XL'].map(s => (
                <button key={s} onClick={() => toggleSize(s)} style={{ padding: '6px 14px', border: '1px solid', fontSize: 12, fontWeight: 600, cursor: 'pointer', borderRadius: 2, borderColor: selectedSizes.includes(s) ? '#000' : '#ddd', background: selectedSizes.includes(s) ? '#000' : '#fff', color: selectedSizes.includes(s) ? '#fff' : '#333' }}>{s}</button>
              ))}
            </div>
          </FilterDropdown>

          <FilterDropdown label={selectedPrice.min > 0 ? selectedPrice.label : 'Khoảng giá'} isOpen={openFilter === 'price'} onToggle={() => toggleFilter('price')}>
            <div style={{ display: 'flex', flexDirection: 'column', gap: 2 }}>
              {PRICE_RANGES.map(range => (
                <label key={range.label} className="filter-checkbox">
                  <input type="radio" name="price" checked={selectedPrice.label === range.label} onChange={() => { setSelectedPrice(range); setOpenFilter(null); }} style={{ accentColor: '#000' }} />
                  {range.label}
                </label>
              ))}
            </div>
          ...
          {hasActiveFilter && (
            <button onClick={() => { setSelectedColors([]); setSelectedSizes([]); setSelectedPrice(PRICE_RANGES[0]); }} style={{ background: 'none', border: 'none', fontSize: 11, color: '#e53935', cursor: 'pointer', fontWeight: 600, textDecoration: 'underline' }}>Xóa bộ lọc</button>
          )}
        </div>
        <div style={{ display: 'flex', alignItems: 'center', gap: 10, fontSize: 13 }}>
          <span style={{ color: '#888', fontWeight: 600, fontSize: 12 }}>Sắp xếp theo:</span>
          <select value={sortBy} onChange={e => setSortBy(e.target.value)} style={{ border: '1px solid #ddd', padding: '6px 12px', fontSize: 13, outline: 'none', cursor: 'pointer', borderRadius: 2, color: '#333' }}>
            {SORT_OPTIONS.map(o => <option key={o.value} value={o.value}>{o.label}</option>)}
          </select>
        </div>
      </div>

      <p style={{ fontSize: 12, color: '#888', marginBottom: 24 }}>Hiển thị <strong style={{ color: '#000' }}>{products.length}</strong> sản phẩm</p>
