...
const AccountPage = ({ user, onLogout, navigate }) => {
  // Nếu chưa đăng nhập, tự động chuyển về login
  if (!user) { navigate('/login'); return null; }
  ...
  return (
    <div style={{ maxWidth: 900, margin: '0 auto', padding: '40px 5% 80px', fontFamily: "'Segoe UI', sans-serif" }}>
      <h1 style={{ fontSize: 26, fontWeight: 800, textAlign: 'center', marginBottom: 48, color: '#111' }}>Tài khoản của bạn</h1>
        ...
        <div>
          ...
          <div style={{ display: 'flex', flexDirection: 'column', gap: 14 }}>
            <div style={{ marginBottom: 10 }}>
              <p style={{ fontSize: 14, fontWeight: 700, margin: '0 0 4px' }}>{user.name}</p>
            ...
            {/* Quyền quản trị (Dành cho Đức và nhân viên) */}
            {(user.isAdmin || user.role) && (
              <span onClick={() => navigate('/admin/dashboard')}
                style={{ fontSize: 13, color: '#1565C0', fontWeight: 700, cursor: 'pointer' }}
              >
                ⚙ Quản trị hệ thống
              </span>
            )}
